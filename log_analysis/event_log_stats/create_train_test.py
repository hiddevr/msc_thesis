...
def trainTestSplit(df, test_len, latest_start, targets):
    ...
    train_case_nrs = case_stops_df[
        case_stops_df["time:timestamp"].values < first_test_start_time].index.array  # added values
...
def nextEventBenchmark(dataset, path, file_name, start_date, end_date, max_days, test_len_share, output_type="xes",
    ...
    dataset_train = dataset[dataset["time:timestamp"] <= pd.to_datetime(separation_time, utc=True)].reset_index(
        drop=True)
